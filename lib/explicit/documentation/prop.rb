...
module Explicit::Documentation
  Prop = ::Data.define(:name, :description, :default, :allowed_values, :nilable, :partial, :locals) do
    def self.from_spec(spec)
      ...
      in [:nilable, subspec]
        from_spec(subspec).with(nilable: true)
      in :string
        just_name("string")
      in [:string, options]
    ...
    def self.just_name(name)
      new(name:, description: nil, default: nil, allowed_values: nil, nilable: nil, partial: nil, locals: nil)
    end

    def self.with_details(name, partial, locals)
      new(name:, description: nil, default: nil, allowed_values: nil, nilable: nil, partial:, locals:)
    end
